...
<?php
$bookingUrl = trim((string) $page->bookingUrl()->value());
$isCalendly = $bookingUrl !== '' && str_contains(strtolower($bookingUrl), 'calendly.com');
$goals = $page->formGoals()->toStructure();
$formHeading = (string) $page->formHeading()->or('Send a message')->value();
?>
...
<main class="flex-1 bg-ink">
    ...
    <section id="contact" class="px-4 py-16 sm:px-6 lg:px-10 lg:py-24">
        <div class="reveal mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1fr_1.05fr] lg:items-start">
            <div>
                ...
                <?php $points = $page->points()->toStructure(); ?>
                ...
                <div class="mt-10 space-y-4 text-sm text-soft">
                    <?php if ($page->email()->isNotEmpty()): ?>
                        <p>
                            ...
                            <a class="mt-1 inline-block text-lime transition hover:brightness-110" href="mailto:<?= esc($page->email()->value()) ?>">
                                <?= esc($page->email()->value()) ?>
                            </a>
                        </p>
                    <?php endif ?>
                    ...
                    <?php if ($page->phone()->isNotEmpty()): ?>
                        <p>
                            ...
                            <span class="mt-1 inline-block text-white/90"><?= esc($page->phone()->value()) ?></span>
                        </p>
                    <?php endif ?>
                </div>
            </div>
            ...
            <div id="contact-form" class="rounded-[1.75rem] border border-white/10 bg-panel p-7 sm:p-8">
                ...
                <?php if (!empty($success)): ?>
                    <p class="mt-4 rounded-2xl border border-lime/30 bg-lime/10 px-4 py-3 text-sm text-lime">
                        <?= esc($page->formSuccess()->or('Thanks — we received your message and will get back to you shortly.')->value()) ?>
                    </p>
                <?php endif ?>

                <?php if (!empty($alert)): ?>
                    <p class="mt-4 rounded-2xl border border-red-400/30 bg-red-500/10 px-4 py-3 text-sm text-red-200">
                        <?= esc($alert) ?>
                    </p>
                <?php endif ?>

                <?php if (empty($success)): ?>
                    <form method="post" action="<?= $page->url() ?>#contact-form" class="mt-6 space-y-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                required
                                value="<?= esc(get('name') ?? '') ?>"
                                class="mt-2 w-full rounded-xl border border-white/10 bg-ink px-4 py-3 text-sm text-white outline-none transition focus:border-lime"
                            >
                        </div>
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    <?php foreach ($goals as $goal): ?>
                                        ...
                                        <?php if ($label !== ''): ?>
                                            <option value="<?= esc($label) ?>" <?= e(get('goal') === $label, 'selected') ?>>
                                                <?= esc($label) ?>
                                            </option>
                                        <?php endif ?>
                                    <?php endforeach ?>
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                rows="5"
                                class="mt-2 w-full rounded-xl border border-white/10 bg-ink px-4 py-3 text-sm text-white outline-none transition focus:border-lime"
                            ><?= esc(get('message') ?? '') ?></textarea>
                        </div>

                        <button type="submit" class="btn-lime inline-flex rounded-full px-7 py-3.5 text-sm font-bold tracking-wide">
                            Send message
                        </button>
                    </form>
                <?php endif ?>
    ...
    <?php if ($bookingUrl !== ''): ?>
        <section id="booking" class="border-t border-white/10 px-4 py-16 sm:px-6 lg:px-10 lg:py-24">
            <div class="reveal mx-auto max-w-7xl">
                <?php if ($page->bookingHeading()->isNotEmpty()): ?>
                    <h2 class="text-center text-3xl font-extrabold tracking-tight text-white sm:text-4xl">
                        <?= esc($page->bookingHeading()->value()) ?>
                    </h2>
                <?php endif ?>

                <?php if ($page->bookingDescription()->isNotEmpty()): ?>
                    <p class="mx-auto mt-3 max-w-2xl text-center text-base leading-7 text-soft">
                        <?= esc($page->bookingDescription()->value()) ?>
                    </p>
                <?php endif ?>
