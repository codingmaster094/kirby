        ...
        <div class="mx-auto mt-14 grid max-w-7xl gap-6 lg:grid-cols-3">
            <?php foreach ($items as $program): ?>
                ...
                <article class="reveal flex h-full flex-col rounded-[1.75rem] border border-white/10 bg-panel p-7">
                    <h3 class="text-2xl font-extrabold tracking-tight text-white"><?= esc($title) ?></h3>
                    <?php if ($summary !== ''): ?>
                    ...
                    <?php if ($ctaText !== '' && $ctaUrl !== ''): ?>
                        <a href="<?= esc($ctaUrl) ?>" class="btn-lime mt-auto inline-flex w-fit rounded-full px-6 py-3 text-sm font-bold tracking-wide"><?= esc($ctaText) ?></a>
                    <?php endif ?>
                </article>
            <?php endforeach ?>
        </div>
